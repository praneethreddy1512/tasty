...
export async function PUT(req: NextRequest, { params }: { params: { id: string, foodId: string } }) {
  await Db();
  ...
  const updated = await RestaurantModel.findOneAndUpdate(
    ...
    { $set: { 'menu.$.name': name, 'menu.$.price': price, 'menu.$.rating': rating, 'menu.$.imgurl': imgurl } },
    { new: true }
  );
  return NextResponse.json(updated);
}

export async function DELETE(req: NextRequest, { params }: { params: { id: string, foodId: string } }) {
  await Db();
